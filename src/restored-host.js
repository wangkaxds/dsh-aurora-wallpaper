...
//       提供场景渲染器打包模块与场景包/shader 路由、向页面注入背景层（视频 + WebGL 场景）与同步脚本。
// 场景渲染支持：通用 pass 管线 + HLSL→GLSL 转译（packages/we-scene）、视频纹理层、
//       父子层级、鼠标视差、性能档位（60/30/30+降采样）与失焦暂停。
//
// 使用前请修改下方 CONFIG。
...
const IMPORT_LINE = /^\s*import\s+.*\bfrom\s+['"][^'"]+['"]\s*;?\s*$/
...
function queryParam(url, name) {
  ...
  const q = raw.indexOf('?')
  if (q < 0) return null
  const part = raw.slice(q + 1).split('&')
...
;(function () {
  ...
  // 性能配置：{mode:'balanced'|'powersave'|'lowend', pauseOnBlur:true}
  function readPerf() {
    try {
      var p = JSON.parse(localStorage.getItem('aurora-perf') || '{}')
      return { mode: p.mode === 'powersave' || p.mode === 'lowend' ? p.mode : 'balanced', pauseOnBlur: p.pauseOnBlur !== false }
    } catch (e) { return { mode: 'balanced', pauseOnBlur: true } }
  }
  var perf = readPerf()
  setInterval(function () { perf = readPerf() }, 500)
  var windowBlurred = false
  window.addEventListener('blur', function () { windowBlurred = true })
  window.addEventListener('focus', function () { windowBlurred = false })
  function stopScene() {
  ...
  function startScene(n) {
    ...
    sceneLoaded = n
    currentPkg = null
    if (renderer && renderer.resetShaderCaches) renderer.resetShaderCaches()
    var pkg = null
    var scene = null
    var textures = new Map()
    window.__auroraScene = {
      getRenderer: function () { return renderer },
      getScene: function () { return scene },
      getTextures: function () { return textures },
      getPkg: function () { return pkg },
      getLoadedIndex: function () { return sceneLoaded },
    }
    fetch('/dyn/aurora-scene/' + n + '.pkg', { cache: 'no-store' }).then(function (r) {
      if (!r.ok) throw new Error('pkg ' + r.status)
      return r.arrayBuffer()
    }).then(function (buf) {
      pkg = lib.parsePkg(new Uint8Array(buf))
      currentPkg = pkg
      scene = lib.parseScene(JSON.parse(new TextDecoder().decode(lib.getEntry(pkg, 'scene.json')).replace(/^\\uFEFF/, '')), null)
      if (!renderer) {
        renderer = lib.createRenderer(cv, { shaderResolver: function (rel) {
          var inner = rel.indexOf('shaders/') === 0 ? rel : 'shaders/' + rel
          var pkgEntry = currentPkg ? lib.getEntry(currentPkg, inner) : null
      ...
      var lastPerfRender = 0
      function frame(now) {
        if (sceneLoaded !== n) return
        var maxFps = perf.mode === 'balanced' ? 60 : 30
        if (perf.pauseOnBlur && windowBlurred) { rafId = requestAnimationFrame(frame); return }
        if (now - lastPerfRender < 1000 / maxFps) { rafId = requestAnimationFrame(frame); return }
        if (!renderBusy) {
          renderBusy = true
          lastPerfRender = now
          if (renderer && renderer.setFboCapFactor) renderer.setFboCapFactor(perf.mode === 'lowend' ? 1 : 0)
          renderer.render(scene, textures, cv.width, cv.height, (now - last) / 1000).catch(function (e) {
            console.error('aurora-scene 渲染失败', e)
          }).then(function () { renderBusy = false })
        }
    ...
    function loadScene(scene, textures) {
      ...
      function loadTex(name) {
        ...
        if (m.video !== undefined) {
          var blob = new Blob([m.video], { type: 'video/mp4' })
          ...
          var ve = { video: videoEl, glTex: lib.makeTexture(renderer.gl, new Uint8Array([0,0,0,0]), 1, 1), width: m.width, height: m.height, rg88: false, lastUploaded: -1 }
          textures.set(name, ve)
          return Promise.resolve(ve)
        }
        if (m.png !== undefined || (m.image !== undefined && m.fif === lib.FIF.JPEG)) {
          var blob2 = new Blob([m.png || m.image], { type: m.png ? 'image/png' : 'image/jpeg' })
          return createImageBitmap(blob2).then(function (bmp) {
        ...
        var e = { glTex: lib.makeTextureMip(renderer.gl, [m], rg88), width: m.width, height: m.height, rg88: rg88 }
        textures.set(name, e)
      ...
      scene.layers.forEach(function (layer) {
        ...
        jobs.push((function () {
          ...
          var material
          if (lib.BUILTIN_MATERIALS[mat.materialPath]) material = lib.BUILTIN_MATERIALS[mat.materialPath]
          else {
  ...
  apply(ctx) {
    ...
    ctx.effect(() => {
      ...
      function serveVideo(v) {
        disposers.push(safeRegister({
          kind: 'exact',
          path: '/dyn/aurora-video/' + v.n + '.mp4',
          handler: async (req, res) => {
            try {
              let bytes
              if (cached !== null && cached.path === v.file) bytes = cached.bytes
              else {
      ...
      async function readProjectInfo(subPath, subName) {
        ...
        } catch (e) { return { title: subName, file: null, type: 'video' } }
      }
      ...
      const boot = (async () => {
        ...
        try {
          ...
          for (const rel of LIB_FILES) {
            ...
            const src = await fs.readText(t)
            const stripped = src.split('\n')
              .filter((line) => !IMPORT_LINE.test(line))
              .join('\n')
            parts.push(stripped)
          }
          libSource = '// we-scene 打包模块（自动拼接，跨文件 import 已剥离）\n' + parts.join('\n// =====\n')
        } catch (e) { libSource = '' }
        disposers.push(safeRegister({
          kind: 'exact',
          path: '/dyn/aurora-lib.js',
          handler: async (req, res) => {
            if (!libSource) { res.writeHead(404); res.end(); return }
        ...
        disposers.push(safeRegister({
          ...
          handler: async (req, res) => {
            let lastErr = '未尝试'
            try {
              const rawUrl = typeof req.url === 'string' ? req.url.split('?')[0] : ''
              const rel = decodeURIComponent(rawUrl).slice('/dyn/aurora-shader/'.length)
              if (!/^[A-Za-z0-9_./-]+$/.test(rel) || rel.includes('..')) { res.writeHead(403); res.end('forbidden'); return }
              ...
              for (const dir of CONFIG.weShaderDirs) {
                try {
                  ...
                  return
                } catch (e2) { lastErr = dir + ': ' + String(e2 && e2.message || e2) }
              }
              res.writeHead(404)
              res.end('shader 未找到: ' + rel + ' | ' + lastErr)
            } catch (err) {
              res.writeHead(500)
              res.end('shader 路由错误: ' + String(err && err.message || err))
            }
          },
        }))
        disposers.push(safeRegister({
          ...
          handler: async (req, res) => {
            try {
              ...
              if (raw !== null) {
                ...
                if (!isNaN(n) && videos.length > 0) current = ((n % videos.length) + videos.length) % videos.length
              }
              res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' })
